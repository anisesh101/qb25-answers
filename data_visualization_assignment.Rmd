...
ggplot(dt, aes(x = Liver)) +
  geom_histogram(breaks = seq(0, 100, 2), fill = "lightgreen", color = "darkgreen", position = "dodge") +
  labs(
    x = "Expression Occurence",
...
#The shape is left skewed. It seems to be the case for most tissues as not all tissues are expressing all genes, only tissue specific ones 
```
...
ggplot(dt, aes(x = Liver + 1, y = `Heart - Left Ventricle` + 1, color = is_mitochondrial)) +
  geom_point(alpha = 0.2, size = 1) +
  scale_x_log10() + 
  scale_y_log10() +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "red") +
